...
function importStudentsAndParents() {
  ...
  // --- 0. CLEANUP OLD DATA ---
  // Clear Students, Enrollments, Classes fully (from Row 2)
  if (sheetStudents.getLastRow() > 1) sheetStudents.deleteRows(2, sheetStudents.getLastRow() - 1);
  if (sheetEnrollments.getLastRow() > 1) sheetEnrollments.deleteRows(2, sheetEnrollments.getLastRow() - 1);
  if (sheetClasses.getLastRow() > 1) sheetClasses.deleteRows(2, sheetClasses.getLastRow() - 1);
  
  // Clear Users but KEEP Admin (if you want to reset fully)
  // For simplicity, we clear all users except the first one (Admin) or just clear and re-add.
  // Let's clear row 2+ for users too.
  if (sheetUsers.getLastRow() > 1) sheetUsers.deleteRows(2, sheetUsers.getLastRow() - 1);
